...
def get_args():
    parser = argparse.ArgumentParser(
        description="train cnn model for despeckling SAR images", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument("--image_dir", type=str, required=True, help="Image directory for input and target images")
    parser.add_argument("--batch_size", type=int, default=32, help="batch size")
    parser.add_argument("--nb_epochs", type=int, default=20, help="total number of epochs")
    ...
    parser.add_argument(
        "--output_path", type=str, default="checkpoints", help="checkpoint directory for saving checkpoints"
    )
    parser.add_argument("--model", type=str, default="red30", help="model architecture")
    parser.add_argument(
        "--min_date_separation", type=int, default=6, help="Minimum date between image pair acquisition"
    )
    parser.add_argument("--logspace", action="store_true", help="Convert images to logspace before training")
...
def main():
    ...
    loss = losses.mean_absolute_error if args.loss == "L1" else losses.mean_squared_error
    ...
    generator = imgloader(
        load_dir, batch_size=batch_size, min_date_separation=min_date_separation, logspace=logspace, verbose=False
    )
    ...
    callbacks.append(
        ModelCheckpoint(
            filepath=checkpoint + "/weights.{epoch:03d}.hdf5", monitor="val_PSNR", verbose=1, mode="max", period=10
        )
    )

    hist = model.fit_generator(
        generator=generator, steps_per_epoch=steps, epochs=nb_epochs, callbacks=callbacks, verbose=1
    )
