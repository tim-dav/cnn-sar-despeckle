...
class imgloader(Sequence):
    ...
    def __init__(self, load_dir, batch_size=4, min_date_separation=6, logspace=False, verbose=False):
        ...
        if logspace:
            print("Imgloader is converting inputs to logspace")
        ...
        for sub_dir in sub_dirs:
            ...
            print("Set %s contains %i images" % (sub_dir, img_count))
                    ...
                    if abs(datediff.days) >= self.min_date_separation:
                        ...
                        if self.verbose:
                            print(
                                "loading pair %s, %s"
                                % (
                                    os.path.join(xfile[self.iset], xfile[self.ifile]),
                                    os.path.join(yfile[self.iset], yfile[self.ifile]),
                                )
                            )
